            ...
            match &mut self.state {
                DefragmentationState::WaitingForNewMessage => {
                    ...
                    if remaining_bytes < 4 {
                        return Err(anyhow!(
                            "First message bytes must be at least 4 bytes to parse length, got {}",
                            remaining_bytes
                        ));
                    }
                    ...
                    if expected_length > 100_000_000 {
                        // 100MB limit
                    ...
                    if expected_length == 0 {
                        self.completed_messages
                            .push_back(CompleteMessage { data: Vec::new() });
                ...
                DefragmentationState::WaitingForData {
                    expected_length,
                    buffer,
                } => {
